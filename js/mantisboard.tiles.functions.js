...
// Contains chart display functions
chartActions = { 
  displayMantisCategories: function(chartCanvas) {
    var dataset={};
  ...
  displayMantisCategoriesResolved: function(chartCanvas) {
  ...
  displayMantisCategoriesBar: function(chartCanvas, chartColors) {
    // Set mantisboard.status.resolvedID according to mantisboard.status.resolvedName and status of the mantis
    var statusResolvedID=getMantisStatusID(mantisboard.status.resolvedName);
  ...
  displayMantisStatus: function(chartCanvas) {
    var dataset={};
    var statusName;
    // Count each Categories type
    $.each(MantisJSON, function(i, item) {
  ...
  displayMantisComposantSANDIE: function(chartCanvas, chartColors) {
    var dataset=[];
    // Count each Categories type for resolved issues
    dataset[0]={};
    dataset[1]={};
